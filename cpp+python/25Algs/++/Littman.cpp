...
int NumDecisionPointsActual = 2;

void Littman(Cell pago[NumDecisionPointsMax][NumDecisionPointsMax], int c[2], int a1[2], int a2[2], int _numDPA) {
	NumDecisionPointsActual = _numDPA;
	...
	//printf("Hacked Nash Bargaining:\n");
	//printf("%i: %.2lf, %.2lf\n", none, p1.x, p1.y);
	//printf("%i: %.2lf, %.2lf\n", ntwo, p2.x, p2.y);

	// find the actions for each player
//	printf("\n");
	for (i = 0; i < NumDecisionPointsActual; i++) {
		for (j = 0; j < NumDecisionPointsActual; j++) {
...
/*
	for (i = 0; i < 2; i++) {
		for (j = 0; j < 2; j++) {
			printf("(%.2lf, %.2lf)   ", payoff1[i][j], payoff2[j][i]);
		}
		printf("\n");
	}
*/
...
void GetDefend(double alpha[NumDecisionPointsMax], double payoff[NumDecisionPointsMax][NumDecisionPointsMax]) {
    ...
    //printf("currentmin = %lf\n", currentmin);
}

void GetAttack(double *alpha, double payoff[NumDecisionPointsMax][NumDecisionPointsMax]) {
