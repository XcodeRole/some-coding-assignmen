...
class HashTable:
    ...
    def get_value(self, key):
        ...
        slot_id=hash(key)%self.number_of_slots
        head = self._data[slot_id]
        if head is None:
            return None
        else:
            self.comparisons_used=self.comparisons_used+1
            while head.key is not key or head is not None:
                self.comparisons_used=self.comparisons_used+1
                head=head.next_node
            if head is None:
                return None
...
def hash_result_finder(tested, quarantined, load_factor=0.5):
    ...
    for elem_t in tested:
        hash_table.store_pair(elem_t[1],(elem_t[0],elem_t[2]))
    #second search in hashtable
    for elem_q in quarantined:
        value_q=hash_table.get_value
        if value_q is None:
            results.append((None,None))
        else:
            results.append(value_q)
