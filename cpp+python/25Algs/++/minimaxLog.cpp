...
minimaxLog::minimaxLog(int _nActions) {//, int _numStates) {
	ms = new double[_nActions];
	mpi = new double[_nActions];

	mv = lowmv = -abs(SENTINAL);
	nott = new bool[_nActions];

	int i;
	for (i = 0; i < _nActions; i++)
		ms[0] = 1.0 / (double)_nActions;
}
